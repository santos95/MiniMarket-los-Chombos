...
persona						int not null,
politica_venta				varchar (3) check (politica_venta in ('CRE','CON'))not null, --crédito, contado
observacion					varchar(200) not null,
fecharegistro				date default getdate(),
estado						char(1)check (estado in ('A','I'))not null,
...
CREATE TABLE tbl_estadocompra-----siguiente
(
...
CONSTRAINT FK_cod_articulo FOREIGN KEY (cod_articulo) REFERENCES cat_articulo (cod_articulo)
)
GO
